...
T = 100
optimizerL = ['SGD', 'Adam', 'RMSprop', 'Adagrad']
domain = [[0, len(optimizerL) - 1]]
algo = VROOM(b=1, f_max=3, domain=domain)
for i in range(T):
    optim_idx = algo.pull(i)[-1]
    model = NeuralNetwork(depth=depth)
    optimizerD = {'SGD': torch.optim.SGD(model.parameters(), lr=lr), 
                  'Adam': torch.optim.Adam(model.parameters(), lr=lr), 
                  'RMSprop': torch.optim.RMSprop(model.parameters(), lr=lr), 
                  'Adagrad': torch.optim.Adagrad(model.parameters(), lr=lr)}
    
    optimizer = optimizerD[optimizerL[int(round(optim_idx))]]
    print('optimizer', optimizerL[int(round(optim_idx))])
...
print('final optimizer:', optimizerL[int(round(algo.get_last_point()[-1]))])
# test
model = NeuralNetwork(depth=depth)
optimizerD = {'SGD': torch.optim.SGD(model.parameters(), lr=lr), 
                  'Adam': torch.optim.Adam(model.parameters(), lr=lr), 
                  'RMSprop': torch.optim.RMSprop(model.parameters(), lr=lr), 
                  'Adagrad': torch.optim.Adagrad(model.parameters(), lr=lr)}
optimizer = optimizerD[optimizerL[int(round(algo.get_last_point()[-1]))]]
for t in range(10):
    print(f"Epoch {t + 1}\n------------------------------")
