...
class NeuralNetwork(nn.Module):
    def __init__(self, depth, hidden=512):
        super().__init__()
        dim = [28 * 28] + [hidden for i in range(depth)] + [10]
...
domain = [[500, 520]]
algo = VROOM(b=1, f_max=5, domain=domain)
for i in range(T):
    print(f"Round {i + 1}\n")
    hidden = int(round(algo.pull(i)[-1]))
    model = NeuralNetwork(depth=depth, hidden=hidden)
...
print('final hidden:', int(round(algo.get_last_point()[-1])))
# test
hidden = int(round(algo.get_last_point()[-1]))
model = NeuralNetwork(depth=depth, hidden=hidden)
optimizer = torch.optim.Adam(model.parameters(), lr=lr)
for t in range(10):
    print(f"Epoch {t + 1}\n------------------------------")
